class Application
  ...
  def call(env)
    ...
    if req.path.match(/gears/) 
      ...
      len = Len.create(usage:0, brand: data["brand"], image: data["image"], focal_length: data["focal_length"], aperture: data["aperture"])
      res_len = {usage:len.usage, id: len.id, brand: len.brand, image: len.image, focal_length: len.focal_length, aperture: len.aperture}
